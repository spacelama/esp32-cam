...
//    vTaskDelay(300 / portTICK_PERIOD_MS); // The LED needs to be turned on ~150ms before the call to esp_camera_fb_get()
    ...
    if (!fb) {
        ...
        syslog.log(LOG_INFO, "Camera capture failed");
...
//  }
    ...
    server.sendHeader("X-Framerate", "60", false);
...
//        vTaskDelay(300 / portTICK_PERIOD_MS); // The LED needs to be turned on ~150ms before the call to esp_camera_fb_get()
           ...
           if(fb->width > 400){
                if(fb->format != PIXFORMAT_JPEG){
                    ...
                    if(!jpeg_converted){
                        ...
                        syslog.log(LOG_INFO, "JPEG compression failed");
                        res = ESP_FAIL;
                    }
                } else {
                    _jpg_buf_len = fb->len;
        ...
        if (!server.client().connected()) {
            syslog.log(LOG_INFO, "esp32cam/stream finished");
            res = ESP_FAIL;
        }
...
#ifdef CONFIG_LED_ILLUMINATOR_ENABLED
    else if(!strcmp(variable.c_str(), "led_intensity")) { led_duty = val; if (isStreaming) app_illuminator_set_led_intensity(led_duty); }
#endif
    else {
        res = -1;
    }
...
void setup_stub() {
    ...
    if (err != ESP_OK) {
        ...
        syslog.logf(LOG_INFO, "Camera init failed with error 0x%x", err);
        return;
    }
